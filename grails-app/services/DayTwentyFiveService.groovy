 ...
 */
class DayTwentyFiveService {

    // http://adventofcode.com/day/25

    final static BigInteger MULT = 252533
    final static BigInteger DIV = 33554393
    final static BigInteger SEED = 20151125
...
Santa looks nervous. Your puzzle input contains the message on the machine's console. What code do you give the machine?
     */
    BigInteger createCodeMatrix(Integer targetRow, Integer targetCol) {

        Integer row = 0
        Integer col = 0
        BigInteger prevValue = SEED

        Integer numTimesHitCol = -1
        Integer maxRow = 0
        BigInteger newVal = SEED

        for(;;){ // infinite for
            if (numTimesHitCol == targetRow) {
                break
            }
            newVal = (prevValue * MULT) % DIV
            if ((row - 1) < 0) {
                ++maxRow
                row = maxRow
                col = 0
