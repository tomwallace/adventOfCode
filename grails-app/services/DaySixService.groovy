...
To defeat your neighbors this year, all you have to do is set up your lights by doing the instructions Santa sent you in order.
    ...
    final String PUZZLE_INPUT = """turn on 887,9 through 959,629
...
turn off 102,229 through 674,529"""
    ...
    LightShow performLightShowAdjustments(String input) {
        LightShow lightShow = new LightShow()

        input.tokenize("\n").each { String line ->
            Map parsedVals = parseInstructionLine(line)
            if (parsedVals.cmd == TURN_ON || parsedVals.cmd == TURN_OFF) {
                lightShow.workOnRange(parsedVals.topLeft, parsedVals.bottomRight, parsedVals.cmd == TURN_ON)
            } else {
                lightShow.toggleRange(parsedVals.topLeft, parsedVals.bottomRight)
            }
        }
    ...
    protected Map parseInstructionLine(String line) {
        // turn on 294,132 through 460,338
        List<String> splitLine = line.tokenize(' ')
        ...
        List<Integer> topLeft = []
        List<String> tl = splitLine[1].tokenize(',')
        topLeft << tl[0].toInteger()
        topLeft << tl[1].toInteger()

        List<Integer> bottomRight = []
        List<String> br = splitLine[3].tokenize(',')
        bottomRight << br[0].toInteger()
        bottomRight << br[1].toInteger()

        return [cmd: cmd, topLeft: topLeft, bottomRight: bottomRight]
    }
     ...
     */
    Integer calcRibbonLength(String input) {
        Integer ribbonLength = 0
        input.tokenize("\n").each { String line ->
            List<Integer> vals = line.trim().tokenize("x")
            Integer l = vals[0].toInteger()
            Integer w = vals[1].toInteger()
            Integer h = vals[2].toInteger()

            // Find side perimeter
            Integer sideOne = (2 * l) + (2 * w)
            Integer sideTwo = (2 * w) + (2 *h)
            Integer sideThree = (2 * h) + (2 * l)
            Integer smallestPerimeter = smallestSideSquareFeet(sideOne, sideTwo, sideThree)

            // Find the area
            Integer area = l * w * h
            Integer length  = smallestPerimeter + area
            ribbonLength += length
        }
        return ribbonLength
    ...
    private Integer smallestSideSquareFeet(Integer s1, Integer s2, Integer s3) {
        if (s1 <= s2 && s1 <= s3) {
            return s1
        }
        if (s2 <= s3 && s2 <= s1) {
    ...
    private List<Integer> removeLargestItem(List<Integer> input) {
        List<Integer> sorted = input.sort[0]
        Integer largest = input.last()
...
class LightShow {
    ...
    LightShow() {
        ...
        List entireOnOff = []
        List entireBrightness = []
        def i = 0
        for (i = 0; i < 1000; i++) {
            List<Boolean> rowOnOff = []
            List<Integer> rowBrightness = []
            def y = 0
            for (y = 0; y < 1000; y++) {
                rowOnOff << false
                rowBrightness << 0
            ...
            entireBrightness << rowBrightness
        }

        gridOnOff = entireOnOff
        gridBrightness = entireBrightness
    }
    ...
    // Determines how many lights are lit
    Integer countBrightness() {
        Integer brightness = 0
        gridBrightness.each { row ->
            row.each { light ->
                brightness += light
            }
        }
    ...
    void workOnLight(Integer row, Integer col, boolean turnOn) {
        gridOnOff[row][col] = turnOn
        gridBrightness[row][col] = gridBrightness[row][col] + (turnOn ? 1 : -1)
        if (gridBrightness[row][col] < 0) {
            gridBrightness[row][col] = 0
        }
    }

    void toggleLight(Integer row, Integer col) {
        boolean existingValue = gridOnOff[row][col]
        gridOnOff[row][col] = !existingValue
        gridBrightness[row][col] = gridBrightness[row][col] +2
    }

    void workOnRange(List<Integer> topLeft, List<Integer> bottomRight, boolean turnOn) {
        def x
        def y
        for (x = topLeft[0]; x <= bottomRight[0]; x++) {
            for (y = topLeft[1]; y <= bottomRight[1]; y++) {
                workOnLight(x, y, turnOn)
            }
        }
    }

    void toggleRange(List<Integer> topLeft, List<Integer> bottomRight) {
        def x
        def y
        for (x = topLeft[0]; x <= bottomRight[0]; x++) {
            for (y = topLeft[1]; y <= bottomRight[1]; y++) {
